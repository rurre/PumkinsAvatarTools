...
namespace Pumkin.PoseEditor
{
                ...
                EditorGUI.BeginDisabledGroup(!selectedAvatar);
                {
                    ...
                    */
                    ...
                    {   
                        if(InPlaymode)
                        {
                            if(addedScene != null && addedScene.isLoaded)
                                ...
                                else
                                {
                                    ...
                                    foreach(var t in ts)
                                    {
                                        if(t.root != t && (!poseOnlySaveChangedRotations || !PumkinsAvatarTools.TransformIsInDefaultPosition(t, true)))
                                            settings.Add(Functions.GetGameObjectPath(t.gameObject), t);
                                    }
    ...
    public class SerialTransform
    {
        ...
        [JsonConstructor]
        public SerialTransform(Vector3 position, Quaternion rotation, Vector3 scale, Vector3 eulerAngles, Vector3 localEulerAngles, Vector3 localScale, Vector3 localPosition, Quaternion localRotation) : base()
        {
